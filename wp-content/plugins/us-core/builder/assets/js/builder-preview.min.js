...
var $usb=parent.$usb,$usbcore=parent.$usbcore||{};['$us','$usbdata'].map(function(name){_window[name]=_window[name]||{}});_window.$ush=_window.$ush||parent.$ush||{};var _DIRECTION_={BOTTOM:'bottom',LEFT:'left',RIGHT:'right',TOP:'top',UNKNOWN:''};var _2D_AXES_={x:'x-axis',y:'y-axis'};var _CONTAINER_={ANY:'any',CHILD:'child',ROOT:'root'};var _MOUSE_EVENT_BUTTON_={MAIN:0};var _$$default={customPrefix:'us_custom_'}
...
_parentTargetContainerId=$usb.builder.getValidContainerId(_parentTargetContainerId);var targetHeight=$ush.parseInt($usbcore.$rect(target).height),borderAround=(children.length&&targetHeight>=50?15:5),borderContainerToCursor=self._getBorderContainerToCursor(self.getElmNode(_parentTargetContainerId),data.clientX,data.clientY,borderAround);if(_DIRECTION_.UNKNOWN!==borderContainerToCursor){_parentTargetContainerId=$usb.builder.getValidContainerId($usb.builder.getElmParentId(_parentTargetContainerId));targetContainer=self.getElmNode(_parentTargetContainerId);targetContainerId=_parentTargetContainerId}}
...
var elmRect=$usbcore.$rect(elm),elmX=(elmRect.x+_window.scrollX),elmY=(elmRect.y+_window.scrollY);elmX-=$ush.parseInt((elm.currentStyle||_window.getComputedStyle(elm)).marginLeft);if($usb.builder.isColumn(baseCurrentId)&&!(data.pageY>=elmY&&data.pageY<=(elmY+elmRect.height))){continue}
...
dragData.lastInsert=$ush.clone(insert);if($usb.builder.isColumn(currentId)){dragData.place=self.getElmNode(currentId)}else{$usbcore.$remove(dragData.place);dragData.place=_document.createElement('div');dragData.place.className='usb_dropplace';var isHorizontalWrapper=($usb.builder.getElmName(targetContainerId)==='hwrapper');if($usb.builder.isRootElmContainer(targetContainerId)||isHorizontalWrapper||dragData.isParentTab){if(isHorizontalWrapper){dragData.place.style.height=$usbcore.$rect(targetContainer).height+'px'}}}
...
var item=self._highlights[elmId],elm=self.getElmNode(elmId);if(!$usbcore.isNode(elm)||(!item.active&&!item.editable)){continue}
var negativeTop=elm.offsetTop<0?abs(elm.offsetTop):0,negativeLeft=elm.offsetLeft<0?abs(elm.offsetLeft):0,elmRect=$usbcore.$rect(elm),cssProps={height:elmRect.height-negativeTop,left:elmRect.left+negativeLeft+(_window.pageXOffset||elm.scrollLeft),top:elmRect.top+negativeTop+(_window.pageYOffset||elm.scrollTop),width:elmRect.width-negativeLeft};$(item.highlight).css(cssProps);$usbcore.$toggleClass(item.highlight,'small',(elmRect.width<75))}},showEditableHighlight:function(id){var self=this;if(!$usb.builder.isValidId(id)){return}
...
var observer=new MutationObserver($ush.debounce(self.setHighlightsPosition.bind(self),1));observer.observe(target,{characterData:!0,childList:!0,subtree:!0});return observer},_createHighlight:function(id){var self=this;if(!$usb.builder.isValidId(id)||self._highlights[id]||!$usbcore.isNode(self.highlight)||$usb.builder.isAliasElmId(id)){return null}
...
var node=self._getNearestNode(e.target);if(!$usbcore.isNode(node)){return}
...
if($usbcore.isNode(self._getNearestNode(e.target))){self.hoveredElm=null;self.hoveredElmId=null}},_elmDuplicate:function(e){var self=this,$highlight=$(e.currentTarget).closest('.usb-hover'),elmId=$highlight.data('elmid');if(!elmId){return}
...
$usbcore.$remove(style)});var getAnimateProps=function(node){if(!$usbcore.isNode(node)){return}
...
return};var node=self.getElmNode(id);if($usb.builder.getElmType(id)==='us_btn'&&$usbcore.isNode(node)){node=node.firstChild||node}
if(!$style){var
...
var cssCode='',collection=self._applyMaskToBackgroundCss(collections[responsiveState]),breakpoint=$usb.config('designOptions.breakpoints.'+responsiveState,'');for(var prop in collection){if(!prop||!collection[prop]){continue}
...
var node=self.getElmNode(id);if($usb.builder.getElmType(id)==='us_btn'&&$usbcore.isNode(node)){node=node.firstChild||node}
if(jsoncss){jsoncss=unescape(''+jsoncss)||'{}'}
for(var prop in specificClasses){var state=(jsoncss.indexOf('"'+prop+'"')>-1);$usbcore.$toggleClass(node,specificClasses[prop],state)}},});$.extend(prototype,{_getBorderContainerToCursor:function(target,clientX,clientY,borderAround){var self=this;if(!$usbcore.isNode(target)||target===self.elmMainContainer||!$.isNumeric(clientX)||!$.isNumeric(clientY)){return _DIRECTION_.UNKNOWN}
clientX+=_window.scrollX;clientY+=_window.scrollY;var elmRect=$usbcore.$rect(target),elmLeft=floor(abs(elmRect.x)+_window.scrollX),elmTop=floor(abs(elmRect.y)+_window.scrollY),elmRight=floor(elmLeft+elmRect.width),elmBottom=floor(elmTop+elmRect.height);if(!$.isNumeric(borderAround)){borderAround=5}
...
return _DIRECTION_.UNKNOWN},_getCurrentCursorDirectionInTarget:function(target,clientX,clientY,axis){if(!$usbcore.isNode(target)){return _DIRECTION_.UNKNOWN}
var self=this,RAD_TO_DEG=180/Math.PI,rect=$usbcore.$rect(target),center={x:rect.width/2+rect.left,y:rect.height/2+rect.top},vector={x:clientX-center.x,y:clientY-center.y},vectorLength=Math.sqrt(vector.x*vector.x+vector.y*vector.y),direction={x:vector.x/vectorLength,y:vector.y/vectorLength},angle=Math.atan2(direction.y,direction.x)*RAD_TO_DEG;if($usbcore.indexOf(axis,_2D_AXES_)<0){axis=_2D_AXES_.y}
if(axis===_2D_AXES_.x){return(angle>-180&&angle<=-130||angle<=180&&angle>130)?_DIRECTION_.LEFT:_DIRECTION_.RIGHT}
return(angle<0)?_DIRECTION_.TOP:_DIRECTION_.BOTTOM},isInsideNode:function(node,pageX,pageY){if(!$usbcore.isNode(node)||$ush.isUndefined(pageX)||$ush.isUndefined(pageY)){return!1}
var nodeRect=$usbcore.$rect(node),offsetTop=(nodeRect.x+_window.scrollX),offsetLeft=(nodeRect.y+_window.scrollY);return(pageX>=offsetTop&&pageX<=(offsetTop+nodeRect.width)&&pageY>=offsetLeft&&pageY<=(offsetLeft+nodeRect.height))},_getNearestNode:function(node,asContainer){var self=this,args=arguments,foundId;if(!$usbcore.isNode(node)){return null}
...
return null},getElmOuterHtml:function(id){var node=this.getElmNode(id);if($usbcore.isNode(node)){return((_document.querySelector('style[data-for="'+id+'"]')||{}).outerHTML||'')+node.outerHTML}
...
var node=self.getElmNode(id);if(!$usbcore.isNode(node)){return}
var rect=$usbcore.$rect(node);if(!(rect.top<0||rect.bottom>(_window.innerHeight||rect.height))){return}
...
nodes.map(function(node){if(!$usbcore.isNode(node)){return}
...
var self=this,node=self.getElmNode(id);if(!$usbcore.isNode(node)){return}
...
args=args||[];self[name].apply(self,$.isArray(args)?args:[args])},vcColumnChange:function(rootContainerId){var self=this;if(!rootContainerId||!$usb.builder.isValidId(rootContainerId)){return}
