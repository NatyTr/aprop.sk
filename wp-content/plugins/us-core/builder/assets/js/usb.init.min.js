...
return result};$usbcore.isNode=function(node){return!!node&&node.nodeType};$usbcore.$rect=function(node){return this.isNode(node)?node.getBoundingClientRect():{}};$usbcore.$addClass=function(node,className){var self=this;if(self.isNode(node)&&className){node.classList.add(className)}
return self};$usbcore.$removeClass=function(node,className){var self=this;if(self.isNode(node)&&className){(''+className).split(/\s/).map(function(itemClassName){if(!itemClassName){return}
node.classList.remove(itemClassName)})}
return self};$usbcore.$toggleClass=function(node,className,state){var self=this;if(self.isNode(node)&&className){self[!!state?'$addClass':'$removeClass'](node,className)}
return self};$usbcore.$hasClass=function(node,className){var self=this;if(self.isNode(node)&&className){var classList=(''+className).split(/\s/);for(var i in classList){className=''+classList[i];if(!className){continue}
if(self.indexOf(className,(''+node.className).split(/\s/))>-1){return!0}}}
return!1};$usbcore.$attr=function(node,name,value){var self=this;if(!self.isNode(node)||!name){return}
if(!$ush.isUndefined(value)){node.setAttribute(name,value);return self}
else if(!!node.getAttribute){return node.getAttribute(name)||''}
return};$usbcore.$remove=function(node){var self=this;if(self.isNode(node)){node.remove()}
...
$usbcore.copyTextToClipboard(self.getElmShortcode(id))},_elmDelete:function(removeId){var self=this;if(!self.isValidId(removeId)){return}
...
if(!$usbcore.isNode(target)){return}
var transit={rect:$usbcore.$rect(target),scrollAcceleration:0,scrollDirection:_undefined,target:target.cloneNode(!0)};$usbcore.$removeClass(transit.target,'hidden');if(isTemplate){$usb.templates.hideTransit()}['width','height'].map(function(prop){var value=ceil(transit.rect[prop]);transit.target.style[prop]=value?value+'px':'auto'});$usbcore.$addClass(transit.target,'elm_transit').$addClass(transit.target,!self.isMode('drag:add')?'state_drag_move':'');_document.body.append(transit.target);_$temp.transit=transit},hasTransit:function(){return!!_$temp.transit},hasDragScrolling:function(){return $usbcore.indexOf((_$temp.transit||{}).scrollDirection,[_DIRECTION_.TOP,_DIRECTION_.BOTTOM])>-1},setTransitPosition:function(pageX,pageY){var self=this;if(!self.hasTransit()||!self.isMode('drag:add','drag:move')){return}
var transit=_$temp.transit||{};if(!$usbcore.isNode(transit.target)){return}
...
var dragData=$usbcore.cache('drag').data();if($usbcore.isNode(dragData.target)){$usbcore.$removeClass(dragData.target,'elm_add_shadow').$removeClass(_document.body,'elm_add_draging')}
...
result[attribute[1]]=(''+(attribute[2]||'')).trim()});return result},buildShortcode:function(shortcode,attsDefaults){if($.isEmptyObject(shortcode)){return ''}
...
return(id.match(_REGEXP_USBID_ALIAS_)||[])[1]||id},getElmType:function(id){var self=this;if($usbcore.isNode(id)){id=self.getElmId(id)}
...
return self._cacheDragProcessData(function(){return(new RegExp('\\['+self.getElmType(id)+'[^\\]]+usbid=\\"'+$ush.escapePcre(id)+'\\"')).test(''+self.pageData.content)},'doesElmExist:'+id,!1)},getElmId:function(node){if(!$usbcore.isNode(node)){return ''}
...
var rect=$usbcore.$rect($item[0]);if(!(rect.top<0||rect.bottom>($body.height()||rect.height))){return}
