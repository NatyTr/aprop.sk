...
return self}}}(jQuery);var _window=window,_document=document,_undefined=undefined;_window.$ush=_window.$ush||{};_window.$usof=_window.$usof||{}
...
return $ush.clone(self._$$data[key]||{})};$usof.field=function(row,options){var self=this;self.$document=$(_document);self.$row=$(row);self.$responsive=$('> .usof-form-row-responsive',self.$row);var data=self.$row.data()||{};self.type=self.$row.usMod('type');self.id=data.id;self.uniqid=$ush.uniqid();self.name=data.name;self.inited=data.inited||!1;self.$input=$('[name="'+data.name+'"]:not(.js_hidden)',self.$row);if(self.inited){return}
...
self._syncSelected();self.$select.trigger('change')},_changeSelect:function(){var self=this,value=self.$select.val();self.setCurrentValue(value);self._syncSelected(value);self._showGridLayoutDesc()},_syncResponsiveValue:function(){this._syncSelected()},_syncSelected:function(value){var self=this;if($ush.isUndefined(value)){value=self.getCurrentValue()}
...
$node=$(node);$node.data('text',$node.text());self._$fontWeights.push($node)})}}});self.$row.on('click','> .usof-form-row-title',self._events.switchSection);if(!self.isEditLive()){var value=self.getValue();$ush.timeout(function(){self.setValue(value)},10)}},_switchSection:function(e){var self=this;self.$row.toggleClass('expand',!self.$row.hasClass('expand')).siblings().removeClass('expand')},_syncAvailableWeights:function(fontFamily){var self=this;for(var i in self._$fontWeights){var $node=self._$fontWeights[i];if($node.hasClass('not-exists')){$node.text($node.data('text')).removeClass('not-exists')}}
...
for(var name in self.fields){var usofField=self.fields[name];if(usofField instanceof $usof.field){var value=$ush.isUndefined(values[name])?usofField.getDefaultValue():values[name];usofField.off('change',self._events.changePropField);usofField.setValue(value,quiet);usofField.on('change',self._events.changePropField)}}
...
self.trigger('data.loaded',res.data.items)},'json')},_initValues:function(){var self=this,loadParams=[],initValues=(self.$value.val()||'').split(self._settings.params_separator)||[];$('.usof-autocomplete-selected',self.$options).remove();$('.selected',self.$list).removeClass('selected');initValues.map(function(key){if(!key){return}
...
var text=$selected.html();return '<span class="usof-autocomplete-selected" title="'+text+'" data-key="'+key+'">\
...
var $target=$('[data-accordion-id]'+hasResponsiveClass,this.$container).next('[data-accordion-content]').find('[data-responsive-state="'+responsiveState+'"]');$target.siblings().removeClass('active');$target.addClass('active').closest('.usof-design-options-content').find('> [data-responsive-state-content]').addClass('hidden').filter('[data-responsive-state-content="'+responsiveState+'"]').removeClass('hidden')},_isValidValue:function(value){try{value=$ush.toPlainObject((''+value).trim());for(var i in this.states){var state=this.states[i];if(state&&!!value[state]){return!0}}}catch(err){}
...
change=(change||JSON.stringify(values)!==JSON.stringify(currentGroupValues[groupName][responsiveState]))}.bind(this));this.$container.find('[data-accordion-id='+groupName+']').toggleClass('changed',change)}.bind(this))},getValue:function(){var value=$.trim(this.$input.val());if(!this.hasStringValue&&value&&typeof value==='string'){value=$ush.toPlainObject(value)}
return value},setValue:function(value,quiet){var savedValues={};if(typeof value==='string'){savedValues=$ush.toPlainObject(value)}else if($.isPlainObject(value)){savedValues=value}
