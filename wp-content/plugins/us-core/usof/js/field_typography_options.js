	...
	$usof.field[ 'typography_options' ] = {
		...
		init: function( options ) {
			...
			self.$row
				...
				.on( 'click', '> .usof-form-row-title', self._events.switchSection );

			// Temporary solution for "Theme Options" page!
			// TODO: Fix as part of the implementation of support for responsive values at the global level
			if ( ! self.isEditLive() ) {
			...
			for ( var name in self.fields ) {
				...
				if ( usofField instanceof $usof.field ) {
					var value = $ush.isUndefined( values[ name ] )
						...
						: values[ name ];
					usofField.off( 'change', self._events.changePropField );
