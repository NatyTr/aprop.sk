...
class WC_SuperFaktura {
	...
	private function __construct() {
		...
		$this->eu_countries = array( 'AT', 'BE', 'BG', 'CY', 'CZ', 'DE', 'DK', 'EE', 'ES', 'FI', 'FR', 'GR', 'HR', 'HU', 'IE', 'IT', 'LT', 'LU', 'LV', 'MT', 'NL', 'PL', 'PT', 'RO', 'SE', 'SI', 'SK' );
	...
	public static function order_number_notice() {
		...
		if ( false !== strpos( $tmpl1 . $tmpl2 . $tmpl3, '[ORDER_NUMBER]' ) ) {
			...
			echo '<div class="notice notice-error is-dismissible"><p><strong>SuperFaktúra Woocommerce</strong>: ' . sprintf( __( 'You use variable %1$s in your invoice nr. or proforma invoice nr., but the plugin "%2$s" is not activated. This may cause that your invoice numbers will not be sequential.', 'woocommerce-superfaktura' ), '[ORDER_NUMBER]', 'WooCommerce Sequential Order Numbers' ) . '</p></div>';
		}
	}
	...
	public function set_order_as_paid() {
		...
		if ( is_numeric( $invoice_id ) && ( false === $user_secret_key || $secret_key === $user_secret_key ) ) {
			...
			$orders = wc_get_orders( array( 'wc_sf_internal_proforma_id' => $invoice_id ) );
			if ( count( $orders ) === 0 ) {
				$orders = wc_get_orders( array( 'wc_sf_internal_regular_id' => $invoice_id ) );
			}
			...
			if ( count( $orders ) === 1 ) {
				...
				if ( 'on-hold' === $order_status ) {
					...
					if ( 'proforma' == $response->{$invoice_id}->Invoice->type ) {
						...
						$regular_id = get_post_meta( $this->get_order_field( 'id', $order ), 'wc_sf_internal_regular_id', true );
						if ( empty( $regular_id ) ) {

							// Schedule an action (because SuperFaktura calls the callback BEFORE it creates the regular invoice automatically).
							as_schedule_single_action( time() + 300, 'sf_fetch_related_invoice', array( 'proforma_id' => $invoice_id ) );
						}
					}
				}
			}
	...
	public function fetch_related_invoice ( $proforma_id ) {
		...
		$orders = wc_get_orders( array( 'wc_sf_internal_proforma_id' => $proforma_id ) );
		if ( empty( $orders ) ) {
			return false;
		}
		...
		// Only if regular invoice does not already exist in WooCommerce.
		$regular_id = get_post_meta( $order_id, 'wc_sf_internal_regular_id', true );
		if ( ! empty( $regular_id ) ) {
			return false;
		}
		...
		if ( 3 == $regular->{$regular_id}->Invoice->status ) {
			delete_post_meta( $order_id, 'wc_sf_payment_link' );
		}

		// Save document ID.
		update_post_meta( $order_id, 'wc_sf_internal_regular_id', $regular_id );

		// Save formatted invoice number.
		update_post_meta( $order_id, 'wc_sf_regular_invoice_number', $regular->{$regular_id}->Invoice->invoice_no_formatted );

		// Save pdf url.
		$language = $this->get_language( $order_id, get_option( 'woocommerce_sf_invoice_language' ), true );
		...
		update_post_meta( $order_id, 'wc_sf_invoice_regular', $pdf );
	...
	public function sf_new_invoice( $order_id ) {
		...
		if ( empty( $payment_method ) && 0.0 === abs( floatval( $this->get_order_field( 'total', $order ) ) ) ) {
		...
		if ( empty( $payment_method ) && class_exists( 'WC_Subscriptions' ) ) {
			$subscription_order_id = get_post_meta( $order_id, '_subscription_renewal', true );
			if ( $subscription_order_id ) {
				$subscription_order = new WC_Order( $subscription_order_id );
				$payment_method     = $this->get_order_field( 'payment_method', $subscription_order );
			}
		}
	 ...
	 */
	private function sf_generate_invoice( $order, $type, $force_create = false ) {
		...
		try {
			...
			if ( empty( $ic_dph ) ) {
				...
				if ( is_plugin_active( 'woocommerce-eu-vat-number/woocommerce-eu-vat-number.php' ) && 'true' === get_post_meta( $this->get_order_field( 'id', $order ), '_vat_number_is_valid', true ) ) {
					...
					if ( empty( $ic_dph ) ) {
						$ic_dph = get_post_meta( $this->get_order_field( 'id', $order ), '_billing_vat_number', true );
					}
				}
			...
			$shipping_phone = $this->get_order_field( 'shipping_phone', $order );
			if ( $shipping_phone ) {
			...
			if ( 'yes' === get_option( 'woocommerce_sf_comments' ) ) {
				...
				if ( ( $client_data['ic_dph'] && WC()->countries->get_base_country() !== $this->get_order_field( 'billing_country', $order ) ) || ( in_array( WC()->countries->get_base_country(), array( 'SK', 'CZ' ), true ) && $this->get_order_field( 'billing_country', $order ) && ! in_array( $this->get_order_field( 'billing_country', $order ), $this->eu_countries, true ) ) ) {
			...
			if ( $country_settings ) {
				...
				foreach ( $country_settings as $country ) {
					if ( $billing_country === $country['country'] ) {
						...
						/* override VAT */
						if ( $country['vat_id'] ) {
							if ( empty( $country['vat_id_only_final_consumer'] ) || ( ! empty( $country['vat_id_only_final_consumer'] ) && empty( $client_data['ic_dph'] ) ) ) {
								$client_country_data['ic_dph'] = $country['vat_id'];
							}
						}
						...
						if ( $country['tax_id'] ) {
							if ( empty( $country['vat_id_only_final_consumer'] ) || ( ! empty( $country['vat_id_only_final_consumer'] ) && empty( $client_data['ic_dph'] ) ) ) {
								$client_country_data['dic'] = $country['tax_id'];
							}
						}
			...
			if ( 'yes' === get_option( 'woocommerce_sf_oss', 'no' ) ) {
				...
				if ( empty( $client_data['ic_dph'] ) && WC()->countries->get_base_country() !== $this->get_order_field( 'billing_country', $order ) && in_array( $this->get_order_field( 'billing_country', $order ), $this->eu_countries, true ) ) {
					$extras['oss'] = true;
				}
			}

			/* packeta */
			$pickup_point_id = get_post_meta( $this->get_order_field( 'id', $order ), 'zasilkovna_id_pobocky', true );
			if ( $pickup_point_id ) {
				$extras['pickup_point_id'] = $pickup_point_id;
			}

			$weight = get_post_meta( $this->get_order_field( 'id', $order ), 'zasilkovna_custom_weight', true );
			if ( empty( $weight ) ) {
				$weight = get_post_meta( $this->get_order_field( 'id', $order ), '_cart_weight', true );
			}
			if ( $weight ) {
	...
	public function checkout_order_meta( $order_id ) {
		update_post_meta( $order_id, 'has_shipping', ( isset( $_POST['shiptobilling'] ) && '1' == $_POST['shiptobilling'] ) ? '0' : '1' );
		...
		if ( isset( $_POST['wi_as_company'] ) && '1' == $_POST['wi_as_company'] ) {
			foreach ( array( 'billing_company_wi_id', 'billing_company_wi_vat', 'billing_company_wi_tax' ) as $key ) {
				if ( isset( $_POST[ $key ] ) ) {
					update_post_meta( $order_id, $key, sanitize_text_field( wp_unslash( $_POST[ $key ] ) ) );
				}
			}
		} else {
	...
	public function woocommerce_process_shop_order_meta( $post_id, $post ) {
		...
		foreach ( array( 'billing_company_wi_id', 'billing_company_wi_vat', 'billing_company_wi_tax' ) as $key ) {
			if ( isset( $_POST[ '_' . $key ] ) ) {
				update_post_meta( $post_id, $key, sanitize_text_field( wp_unslash( $_POST[ '_' . $key ] ) ) );
			}
		}
	}
	...
	public function add_meta_boxes() {
		add_meta_box( 'wc_sf_invoice_box', __( 'Invoices', 'woocommerce-superfaktura' ), array( $this, 'add_box' ), 'shop_order', 'side' );
	}
	...
	private function sf_can_regenerate( $order ) {
		...
		if ( 'completed' === $this->get_order_field( 'status', $order ) ) {
			$can_regenerate = false;
		}

		if ( 'processing' === $this->get_order_field( 'status', $order ) && 'cod' !== $this->get_order_field( 'payment_method', $order ) ) {
			$can_regenerate = false;
		}
	...
	public function add_box( $post ) {
		...
		if ( ! empty( $proforma ) ) {
			$error_html = sprintf( '%s<br><a href="%s">%s</a>', __( 'Proforma could not be found in SuperFaktura.', 'woocommerce-superfaktura' ), admin_url( 'admin.php?sf_invoice_proforma_create=1&force_create=1&sf_order=' . $post->ID ), __( 'Create new proforma invoice', 'woocommerce-superfaktura' ) );
			...
			echo wp_kses( '<p><a href="' . admin_url( 'admin.php?sf_invoice_proforma_create=1&sf_order=' . $post->ID ) . '">' . __( 'Create proforma invoice', 'woocommerce-superfaktura' ) . '</a></p>', $this->allowed_tags );
		}

		if ( ! empty( $invoice ) ) {
			$error_html = sprintf( '%s<br><a href="%s">%s</a>', __( 'Invoice could not be found in SuperFaktura.', 'woocommerce-superfaktura' ), admin_url( 'admin.php?sf_invoice_regular_create=1&force_create=1&sf_order=' . $post->ID ), __( 'Create new invoice', 'woocommerce-superfaktura' ) );
			...
			echo wp_kses( '<p><a href="' . admin_url( 'admin.php?sf_invoice_regular_create=1&sf_order=' . $post->ID ) . '">' . __( 'Create invoice', 'woocommerce-superfaktura' ) . '</a></p>', $this->allowed_tags );
		}

		if ( ! empty( $cancel ) ) {
			$error_html = sprintf( '%s<br><a href="%s">%s</a>', __( 'Credit note could not be found in SuperFaktura.', 'woocommerce-superfaktura' ), admin_url( 'admin.php?sf_invoice_cancel_create=1&force_create=1&sf_order=' . $post->ID ), __( 'Create new credit note', 'woocommerce-superfaktura' ) );
			...
			echo wp_kses( '<p><a href="' . admin_url( 'admin.php?sf_invoice_cancel_create=1&sf_order=' . $post->ID ) . '">' . __( 'Create credit note', 'woocommerce-superfaktura' ) . '</a></p>', $this->allowed_tags );
		}

		if ( ( ! empty( $proforma ) || ! empty( $invoice ) ) && $this->sf_can_regenerate( $order ) ) {
			echo wp_kses( '<p><a href="' . esc_url( admin_url( 'admin.php?sf_regen=1&sf_order=' . $post->ID ) ) . '">' . __( 'Regenerate existing invoices', 'woocommerce-superfaktura' ) . '</a></p>', $this->allowed_tags );
		}
	...
	public function sf_invoice_link_page( $order_id ) {
		if ( 'yes' === get_option( 'woocommerce_sf_order_received_invoice_link', 'yes' ) ) {
			...
			$pdf = get_post_meta( $order_id, 'wc_sf_invoice_regular', true );
			if ( $pdf ) {
			...
			$pdf = get_post_meta( $order_id, 'wc_sf_invoice_proforma', true );
			if ( $pdf ) {
	...
	public function sf_invoice_link_email( $order, $sent_to_admin = false ) {
		...
		if ( 'cod' === $this->get_order_field( 'payment_method', $order ) && 'yes' === get_option( 'woocommerce_sf_cod_email_skip_invoice', 'no' ) ) {
			return;
		}
	 ...
	 * @param WC_Order $order Order.
	 */
	private function get_order_field( $id, $order ) {
	 ...
	 */
	private function version_check( $version = '3.2' ) {
	...
	private function order_is_paid( $order ) {
		...
		if ( class_exists( 'WC_Order_Status_Manager_Order_Status' ) ) {
			...
			if ( false !== $set_as_paid_statuses ) {
				if ( in_array( $this->get_order_field( 'status', $order ), $set_as_paid_statuses, true ) ) {
					$is_paid = true;
				}
			} else {
